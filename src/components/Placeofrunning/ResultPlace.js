...
const ResultPlace = (props) => {
    ...
    const time = (timeH * 3600 + timeMin * 60 + timeSec) / 3600;
    const speed = Math.round((distance / time) * 1000) / 1000;

    const timeKm = Math.floor((timeH * 3600 + timeMin * 60 + timeSec) / distance);
    ...
    return (
        <>
            <h1>{timeH + timeMin + timeSec > 0 && distanceKm + distanceM + selectDistance > 0 ? `Twoje tempo: ${timeMinut}' ${timeSekund}" /km, Twoja prędkość: ${speed} km/h` : "PROSZĘ PODAĆ PRAWIDŁOWE DANE"} </h1>
